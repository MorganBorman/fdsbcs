...
ENetSocket localmastersock = ENET_SOCKET_NULL;
ENetAddress localmasteraddress = { ENET_HOST_ANY, ENET_PORT_ANY };
int lastupdatelocalmaster = 0;
vector<char> localmasterout, localmasterin;
int localmasteroutpos = 0, localmasterinpos = 0;
VARN(updatelocalmaster, allowupdatelocalmaster, 0, 1, 1);
...
void disconnectlocalmaster()
{
    ...
    localmasteroutpos = masterinpos = 0;

    localmasteraddress.host = ENET_HOST_ANY;
    localmasteraddress.port = ENET_PORT_ANY;

    lastupdatelocalmaster = 0;
}

SVARF(localmastername, server::defaultlocalmaster(), disconnectlocalmaster());
VARF(localmasterport, 1, server::masterport(), 0xFFFF, disconnectlocalmaster());
...
ENetSocket connectlocalmaster()
{
    if(!localmastername[0]) return ENET_SOCKET_NULL;
...
void processlocalmasterinput()
{
    ...
    while(end)
    {
        *end++ = '\0';
        ...
        if(!strncmp(input, "failreg", cmdlen))
            conoutf(CON_ERROR, "local master server registration failed: %s", args);
        else if(!strncmp(input, "succreg", cmdlen))
            conoutf("local master server registration succeeded");
        else server::processlocalmasterinput(input, cmdlen, args);
...
void flushlocalmasteroutput()
{
    ...
    if(sent >= 0)
    {
        ...
        if(localmasteroutpos >= masterout.length())
        {
            localmasterout.setsize(0);
            localmasteroutpos = 0;
        }
...
void checkserversockets()        // reply all server info requests
{
    ...
    if(localmastersock != ENET_SOCKET_NULL)
    {
        maxsock = max(maxsock, localmastersock);
        ENET_SOCKETSET_ADD(sockset, localmastersock);
    }
...
void updatelocalmasterserver()
{
    if(localmastername[0] && allowupdatelocalmaster) requestlocalmasterf("regserv %d\n", serverport);
    lastupdatelocalmaster = totalmillis ? totalmillis : 1;
}
...
void serverslice(bool dedicated, uint timeout)   // main server update, called from main loop in sp, or from below in dedicated server
{
    ...
    if(!lastupdatelocalmaster || totalmillis-lastupdatelocalmaster>60*60*1000)       // send alive signal to masterserver every hour of uptime
        updatelocalmasterserver();
